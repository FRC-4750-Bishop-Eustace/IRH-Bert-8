...
public class IntestineTransport extends SubsystemBase {
  ...
  public float IR4Count; // counts number that passer IR4
  ...
  double storageConveyor2Speed = 0.90;
 /*
...
public void Indexer(){
  ...
  if(getIR2()){
    ...
    if(IR3Count > 2){
      transportTimer = System.currentTimeMillis();
      while((System.currentTimeMillis() - transportTimer) <= 800){
        storageConveyor2.set(storageConveyor2Speed);
        if ((System.currentTimeMillis() - transportTimer) >= 800){
          storageConveyor2.set(0);
        }
      }
    }
...
public void completeShooting(){
  ...
  if(getIR4() && IR4Count >= 2){
    ...
    while((System.currentTimeMillis() - shooterTimer) <= 600){
      storageConveyor2.set(storageConveyor2Speed);
      lastBallTimer = System.currentTimeMillis();
      while((System.currentTimeMillis() - lastBallTimer <= 300)){
      intakeConveyor1.set(intakeConveyor1Speed);
      }
      }
    }
